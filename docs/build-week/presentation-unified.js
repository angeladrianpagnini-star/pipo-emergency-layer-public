(function () {
  ...
  const copy = {
    "es-AR": {
      documentTitle: "PIPO Emergency Layer - Presentación operativa unificada", navCitizen: "Ciudadano", navCoordination: "Coordinación", navConsoles: "Consolas", navField: "Campo", navJustice: "Fiscalía y denuncias", navDocs: "Documentación", navClosure: "Cierre", all: "Todas las consolas",
      ...
      justiceTitle: "Consulta de campo a Fiscalía", justiceNotice: "Flujo conceptual. No constituye actuación fiscal real ni orden judicial.", consultation: "Consulta", urgency: "Urgencia", reference: "Referencia ficticia", sendConsultation: "Enviar consulta simulada", simulateResponse: "Simular respuesta de Fiscalía", received: "Recibida", answered: "Respondida", completed: "Cerrada", prosecutorAnswer: "Preservar el lugar, ampliar información y registrar la actuación. Respuesta simulada.", stationTitle: "Consola de Comisaría", stationNotice: "Demostración conceptual. No constituye denuncia policial real.", stationScope: "La comisaría agrega su propia referencia al expediente sin modificar el registro ciudadano ni las actas de otros organismos.", municipalTitle: "147 — Atención y respuesta municipal", municipalNotice: "Ejemplo conceptual, no integrado con un municipio real.", municipalCategory: "Categoría simulada", municipalRoute: "Área asignada", municipalTime: "Plazo estimado", municipalClose: "Cierre ciudadano",
    ...
    "en-US": {
      documentTitle: "PIPO Emergency Layer - Unified operational presentation", navCitizen: "Citizen", navCoordination: "Coordination", navConsoles: "Consoles", navField: "Field", navJustice: "Prosecution and reports", navDocs: "Documentation", navClosure: "Closure", all: "All consoles",
      ...
      justiceTitle: "Field consultation to prosecution", justiceNotice: "Conceptual flow. It is not a real prosecutorial action or judicial order.", consultation: "Consultation", urgency: "Urgency", reference: "Fictional reference", sendConsultation: "Send simulated consultation", simulateResponse: "Simulate prosecution response", received: "Received", answered: "Answered", completed: "Closed", prosecutorAnswer: "Preserve the scene, expand the information, and record the action. Simulated response.", stationTitle: "Police station console", stationNotice: "Conceptual demonstration. It is not a real police report.", stationScope: "The police station adds its own reference to the record without modifying the citizen record or other agencies' acts.", municipalTitle: "147 — Municipal attention and response", municipalNotice: "Conceptual example; it is not connected to a real municipality.", municipalCategory: "Simulated category", municipalRoute: "Assigned area", municipalTime: "Estimated timeframe", municipalClose: "Citizen closure",
  ...
  function renderHero() {
    return `<section class="presentation-hero" aria-labelledby="presentationTitle">
      <div class="presentation-hero-copy"><p class="presentation-kicker">${t("eyebrow")}</p><h1 id="presentationTitle">${t("title")}</h1><h2>${t("claim")}</h2><p>${t("lead")}</p>
        <div class="presentation-actions"><button type="button" class="presentation-primary" data-unified-action="start-tour">${t("start")}</button><a class="presentation-secondary" href="#presentationCitizen">${t("fullFlow")}</a><button type="button" class="presentation-secondary" data-unified-action="advanced">${t("advanced")}</button><button type="button" class="presentation-icon-action" data-unified-action="restart" title="${t("restart")}" aria-label="${t("restart")}">↺</button></div>
      </div>
  ...
  function renderCommunications() {
    const rows = state.routed ? [
      ["COM-DEMO-001", consoleName("health"), consoleName("traffic"), t("supportRequest"), state.locale === "es-AR" ? "Se solicita corredor sanitario." : "A sanitary corridor is requested.", t("read")],
      ["COM-DEMO-002", consoleName("security"), consoleName("fire"), t("operationalMessage"), state.locale === "es-AR" ? "Se informa posible derrame con riesgo de incendio." : "Possible spill with fire risk reported.", t("acceptance")],
      ["COM-DEMO-003", consoleName("prosecution"), consoleName("security"), t("diligenceRequest"), state.locale === "es-AR" ? "Preservar el lugar y ampliar información." : "Preserve the scene and expand the information.", t("response")],
      ["COM-DEMO-004", t("master"), t("all"), t("priorityUpdate"), state.locale === "es-AR" ? "Prioridad actualizada y recursos confirmados." : "Priority updated and resources confirmed.", t("read")],
    ] : [];
    return `<section class="presentation-section presentation-communications" aria-labelledby="communicationsTitle"><div class="presentation-section-heading"><div><p class="presentation-kicker">${t("communications")}</p><h2 id="communicationsTitle">${t("commTitle")}</h2><p>${t("commLead")}</p></div></div><div class="presentation-communications-table" role="region" aria-label="${t("commTitle")}"><div class="communications-row communications-head"><span>${t("communicationId")}</span><span>${t("sender")}</span><span>${t("recipient")}</span><span>${t("purpose")}</span><span>${t("status")}</span></div>${rows.length ? rows.map((row) => `<article class="communications-row"><strong>${row[0]}</strong><span>${row[1]}</span><span>${row[2]}</span><span><b>${row[3]}</b>${row[4]}</span><span>${row[5]} · ${t("integrity")}</span></article>`).join("") : `<p class="presentation-empty">${t("pending")}</p>`}</div></section>`;
  }
  ...
  function renderAdvanced() {
    return `<details id="presentationAdvanced" class="presentation-advanced" ${state.filming ? "" : ""}><summary>${t("advancedTitle")}</summary><div><p>${t("advancedLead")}</p><p>${t("v36")}</p><section><h3>${t("configTitle")}</h3><p>${t("configLead")}</p><div class="presentation-config-list">${config.alerts.map((item) => `<article><strong>${label(item)}</strong><span>${item.route.map(consoleName).join(" → ")}</span></article>`).join("")}</div></section><section><h3>${t("limitsTitle")}</h3><ul>${t("limits").map((item) => `<li>${item}</li>`).join("")}</ul></section></div></details>`;
  }
  ...
  function bindEvents() {
    root.querySelectorAll("[data-unified-action]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (action === "advanced") { const details = document.getElementById("presentationAdvanced"); if (details) { details.open = true; details.scrollIntoView({ behavior: "smooth", block: "start" }); } return; }
